...
def main():
    ...
    global motion_test_active, motion_phase, motion_start_time
    global motion_coordinates, motion_vectors
    ...
    print("  1. 2초 대기")
    print("  2. 3초 카운트다운")
    print("  3. 움직임 이동 단계 (3초)")
    print("  4. 프레임 간 이동 벡터 분석")
    print("=" * 70)
    print()
    ...
    motion_test_active = True
    motion_phase = "waiting"  # waiting → moving → done
    ...
    try:
        ...
        while True:
            ...
            # ⭐⭐⭐ 카운트다운 로직 (메인 루프 안에서!) ⭐⭐⭐
            if motion_test_active:
                # 2초 대기
                if not countdown_printed["2sec_wait"] and elapsed_test >= 2:
                    print("⏱️  3초")
                    countdown_printed["2sec_wait"] = True
                    countdown_printed["3sec"] = True

                # 3초
                if not countdown_printed["2sec"] and elapsed_test >= 3:
                    print("⏱️  2초")
                    countdown_printed["2sec"] = True

                # 2초
                if not countdown_printed["1sec"] and elapsed_test >= 4:
                    print("⏱️  1초")
                    countdown_printed["1sec"] = True

                # 1초 → 움직임 시작!
                if not countdown_printed["move_start"] and elapsed_test >= 5:
                    print("\n🚀 움직임 이동 시작!\n")
                    motion_phase = "moving"
            ...
            if motion_phase == "moving":
                elapsed_move = now - motion_start_time
                ...
                if elapsed_move < 3.0:
                    ...
                    if len(face_boxes_preFrame) > 0:
                        ...
                        motion_coordinates.append((now, box_cx_temp, box_cy_temp))

                        # 이전 프레임과의 벡터 계산
                        if len(motion_coordinates) >= 2:
                            prev_time, prev_cx, prev_cy = motion_coordinates[-2]
                            curr_time, curr_cx, curr_cy = motion_coordinates[-1]

                            # 이동 벡터 크기 계산: d = √((Δx)² + (Δy)²)
                            delta_x = curr_cx - prev_cx
                            delta_y = curr_cy - prev_cy
                            vector_magnitude = np.sqrt(delta_x**2 + delta_y**2)

                            motion_vectors.append(vector_magnitude)

                            debug_log(f"벡터 크기: {vector_magnitude:.2f}px (Δx={delta_x}, Δy={delta_y})", "DETAIL")

                    # 진행 상황 출력 (1초마다)
                    if int(elapsed_move) > int(elapsed_move - 0.033):
                        remaining = 3.0 - elapsed_move
                        if remaining > 0:
                            print(f"⏱️  남은 시간: {remaining:.1f}초 (좌표 {len(motion_coordinates)}개 수집)")

                # 3초 경과 시 결과 분석
                elif elapsed_move >= 3.0:
                    motion_test_active = False
                    motion_phase = "done"
                    ...
                    if len(motion_vectors) > 0:
                        # 10px 이하인 벡터 개수 계산
                        stable_frames = sum(1 for v in motion_vectors if v <= MOTION_THRESHOLD)
                        total_frames = len(motion_vectors)
                        stable_ratio = (stable_frames / total_frames * 100) if total_frames > 0 else 0

                        # 통계 계산
                        vectors_array = np.array(motion_vectors)

                        print("=" * 70)
                        print("📊 움직임 벡터 분석 결과")
                        print("=" * 70)
                        print(f"📍 수집된 좌표 개수: {len(motion_coordinates)}개")
                        print(f"📏 계산된 벡터 개수: {total_frames}개")
                        print(f"🎯 임계값: {MOTION_THRESHOLD}px")
                        print()
                        print(f"📈 벡터 크기 통계:")
                        print(f"  평균: {vectors_array.mean():.2f}px")
                        print(f"  중앙값: {np.median(vectors_array):.2f}px")
                        print(f"  최소: {vectors_array.min():.2f}px")
                        print(f"  최대: {vectors_array.max():.2f}px")
                        print(f"  표준편차: {vectors_array.std():.2f}px")
                        print()
                        print(f"✅ {MOTION_THRESHOLD}px 이하 프레임: {stable_frames}개")
                        print(f"❌ {MOTION_THRESHOLD}px 초과 프레임: {total_frames - stable_frames}개")
                        print(f"📊 안정 프레임 비율: {stable_ratio:.2f}%")
                        print("=" * 70)
                        print()

                        # 평가
                        if stable_ratio >= 85:
                            print("✅ 우수: 매우 안정적인 추적 (85% 이상)")
                        elif stable_ratio >= 60:
                            print("🟢 양호: 안정적인 추적 (60~79%)")
                        elif stable_ratio >= 40:
                            print("🟡 보통: 추적 성능 개선 필요 (40~59%)")
                        else:
                            print("🔴 불량: 추적 안정성 낮음 (40% 미만)")

                        print("\n정상 모드로 전환합니다...\n")
                    else:
                        print("⚠️  테스트 실패: 벡터를 계산하지 못했습니다.")
                        print("   얼굴이 검출되지 않았거나 프레임이 부족합니다.\n")
            # ⭐⭐⭐ 움직임 분석 테스트 처리 끝 ⭐⭐⭐
            ...
            # ⭐ 테스트 모드 상태 표시
            if motion_phase == "moving" and motion_start_time:
                elapsed_move = now - motion_start_time
                test_text = f"움직임 분석 중... {elapsed_move:.1f}s / 3.0s (벡터 {len(motion_vectors)}개)"
            ...
            cv2.imshow("Face Tracking Robot - Motion Analysis", display)
