...
def main():
    ...
    global debug_counters, tracking_test_mode
    ...
    countdown_printed = {
        ...
        "move": False, "move1": False, "move2": False, "check": False
    }
    ...
    print("  3. '움직임 시작' 신호 후 움직이세요")
    print("  4. 2초 후 로봇팔이 따라왔는지 확인합니다")
    print("=" * 70)
    print()
    ...
    try:
        ...
        while True:
            ...
            if movement_start_time and not countdown_printed["move1"] and (now - movement_start_time) >= 1:
                print("⏱️  1초 경과")
                countdown_printed["move1"] = True

            if movement_start_time and not countdown_printed["move2"] and (now - movement_start_time) >= 2:
                print("⏱️  2초 경과")
                countdown_printed["move2"] = True

            if movement_start_time and not countdown_printed["check"] and (now - movement_start_time) >= 2.5:
                print("⏱️  2초 경과 - 추적 결과 확인 중...\n")
                countdown_printed["check"] = True

            # ⭐ 얼굴 검출 체크 (2초 후)
            if movement_start_time and not face_detection_checked and (now - movement_start_time) >= 2.5:
                face_detection_checked = True
                tracking_test_mode = False

                # 현재 프레임에서 얼굴 검출 시도
                temp_faces = detect_faces_dnn(frame)
                face_detected = len(temp_faces) > 0

                print("=" * 70)
                print("📊 추적 테스트 결과")
                print("=" * 70)
                print(f"⏱️  경과 시간: 2초")
                print(f"🎯 얼굴 검출 여부: {face_detected}")

                if face_detected:
                    print("✅ 성공: 로봇팔이 사용자를 성공적으로 추적했습니다!")
                    print("   → 2초 안에 얼굴이 카메라 영역 내에 유지되었습니다.")
                else:
                    print("❌ 실패: 로봇팔이 사용자를 놓쳤습니다!")
                    print("   → 2초 후 얼굴이 카메라 영역을 벗어났습니다.")
                    print("   💡 가능한 원인:")
                    print("      - 로봇팔 속도가 느림")
                    print("      - 사용자가 너무 빠르게 이동")
                    print("      - 모터 제어 파라미터 조정 필요")
            ...
            # ⭐ 추적 테스트 모드 표시
            if tracking_test_mode and movement_start_time:
                elapsed_move = now - movement_start_time
                test_text = f"TEST: {elapsed_move:.1f}s"
            ...
            cv2.imshow("Face Tracking Robot - Tracking Test Mode", display)
