...
# - 회전/전단 보정 없음: 오직 translation + uniform zoom (왜곡 0)
# - 줌 울렁임 제거: 히스테리시스 + 양자화 + OneEuro + 슬루 제한
# - 얼굴은 항상 화면 정중앙(화면이 따라감)
# - 저지연 캡처 스레드(1080p@60fps, MJPG, grab()로 버퍼 드롭)
# - DNN(SSD ResNet10) + 칼만(x,y,vx,vy) + 예측 lead
# - 저장본/미리보기 모두 미러 없음
# - 네 요구대로 1920x1080, CAP_DSHOW 사용으로 통합
# ============================================================
...
ZOOM_MIN, ZOOM_MAX = 1.00, 1.70
ZOOM_SLEW_PER_S = 0.55    # 초당 줌 변화 한계
ZOOM_QUANT = 0.015        # 줌 양자화(계단)로 미세 떨림 제거
ZOOM_HYST = 0.06          # 히스테리시스 밴드(±6%)

BASE_MARGIN_FRAC = 0.10    # (미사용) 초기 여백 파라미터 자리
SAFETY_PX = 8              # 크롭 경계 여유
...
class CaptureThread:
    def __init__(self, cam_index=0, backend=cv2.CAP_DSHOW):
        ...
        # 오토 기능은 장치에 따라 조절
        try: self.cap.set(cv2.CAP_PROP_AUTO_EXPOSURE, 1)  # 일부 백엔드에서 1=수동
        except: pass
    ...
    try:
        ...
        while True:
            ...
            if face_found and bh > 0:
                ...
                z_desired = float(np.clip(desired_face_px / float(bh), ZOOM_MIN, ZOOM_MAX))
            else:
                z_desired = 1.0
            ...
            guide_w = int(0.32 * w)
            guide_h = int(guide_w * 1.15)  # 얼굴 비율 가이드
