...
def compute_motor_angles(center_x, center_y, area, frame_shape, desired_area=42000, kf=None, kalman_inited=False):
    ...
    # Kalman 속도 정보 활용 (선행 예측)
    predict_ahead = 0.15  # 0.15초 후 위치 예측
    if kalman_inited and kf is not None:
        vx_normalized = kf.statePost[2,0] / (w/2)  # 속도 정규화
        vy_normalized = kf.statePost[3,0] / (h/2)

        # 예측 보상 추가
        error_x += vx_normalized * predict_ahead
        error_y += vy_normalized * predict_ahead

    # PD 제어 게인
    Kp_xy = 90 * 0.35      # 비례 게인 (기본 응답)
    Kd_xy = 90 * 0.25      # 미분 게인 (빠른 움직임에 강하게 반응)

    Kp_area = 0.0020       # 거리 비례 게인
    Kd_area = 0.0012       # 거리 미분 게인

    # PD 제어 출력
    output_x = Kp_xy * error_x + Kd_xy * d_error_x
    output_y = Kp_xy * error_y + Kd_xy * d_error_y
    output_area = Kp_area * error_area + Kd_area * d_error_area

    # 이전 오차 저장 (다음 프레임 미분 계산용)
    ...
    return {
        "motor_1": output_y,
        "motor_2": output_y,
        "motor_3": output_y * 0.5,
        "motor_4": output_area,
        "motor_5": output_y * 0.2,
        "motor_6": output_x * 0.5
    }
