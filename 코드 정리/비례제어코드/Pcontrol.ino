...
// ---------------------- CONFIG ----------------------
#define AXIS_COUNT 6
#define DEBUG_SERIAL false  // true로 변경하면 상세 디버깅 출력

// DS3230 PRO180 pulse range: 500–2500us at 50Hz (same as MG996R)
...
bool moving() {
  ...
  for (int i = 0; i < AXIS_COUNT; ++i) {
    ...
    if (npulse != curr_pulse[i]) {
      curr_pulse[i]  = npulse;
      curr_degree[i] = ndeg;
      pca9685.setPWM(i, 0, npulse); // 변경 시만 전송
      anyMoved = true;
    }
  }
