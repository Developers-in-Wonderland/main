...
# ============================================================
# 방법 A: 비례 제어 (Proportional Control)
# ============================================================
def compute_motor_angles_safe(center_x, center_y, area, frame_shape):
    """
    적응형 비례 제어 + Adaptive EMA: 지연 없이 부드러운 모터 제어
    - 거리 기반 적응형 alpha (멀리: 빠름, 가까이: 부드러움)
    - 거리 기반 가변 delay (멀리: 빠른 갱신, 가까이: 안정화)
    - Soft deadzone (경계에서 부드러운 전환)
    """
    ...
    # 비례 제어 게인
    Kp_xy = 0.02  # XY축 비례 게인
    Kp_z = 0.00003  # Z축 비례 게인
    ...
    def soft_deadzone(error, deadzone):
        ...
        if abs_error <= deadzone:
            ...
            return error * ratio * 0.5  # 최대 50%까지만 출력
        else:
            return error
    ...
    # Adaptive Alpha (거리 기반)
    if distance > 150:
        alpha = 0.6  # 멀리: 빠른 반응 (지연 최소화)
        delay = 10   # 빠른 갱신
        category = "멀리"
    elif distance > 80:
        alpha = 0.4  # 중간: 균형
        ...
        alpha = 0.2  # 가까이: 부드러움 우선 (떨림 최소화)
        delay = 15   # 느린 갱신 (안정화)
        category = "가까이"

    # EMA 필터 (Adaptive)
    if not hasattr(compute_motor_angles_safe, 'ddx_prev'):
        compute_motor_angles_safe.ddx_prev = 0
        compute_motor_angles_safe.ddy_prev = 0
        compute_motor_angles_safe.ddz_prev = 0

    ddx = alpha * ddx_raw + (1 - alpha) * compute_motor_angles_safe.ddx_prev
    ddy = alpha * ddy_raw + (1 - alpha) * compute_motor_angles_safe.ddy_prev
    ddz = alpha * ddz_raw + (1 - alpha) * compute_motor_angles_safe.ddz_prev

    # 저장
    compute_motor_angles_safe.ddx_prev = ddx
    compute_motor_angles_safe.ddy_prev = ddy
    compute_motor_angles_safe.ddz_prev = ddz

    debug_log(f"적응제어[{category}]: 거리={distance:.0f}px, alpha={alpha:.1f}, delay={delay}ms, "
              f"오차=({dx:+.0f},{dy:+.0f}), raw=({ddx_raw:+.2f},{ddy_raw:+.2f}), "
              f"출력=({ddx:+.2f},{ddy:+.2f})", "DETAIL")
...
def main():
    ...
    # 표시 스무딩 - 멀미 방지 (강한 필터)
    cx_oe = OneEuro(0.5, 0.1, 1.0)  # 떨림 완전 제거
    cy_oe = OneEuro(0.5, 0.1, 1.0)

    # 모터 제어 스무딩 - 부드러운 추적 (강한 필터)
    motor_cx_oe = OneEuro(0.5, 0.1, 1.0)  # 화면과 동일, 떨림 제거
    ...
    try:
        ...
        while True:
            ...
            if face_found and move_ready.is_set() and should_track:
                ...
                # 🔍 필터 진단 로그 (0.5초마다)
                if not hasattr(motor_cx_oe, '_last_log_time'):
                    motor_cx_oe._last_log_time = 0
                if now - motor_cx_oe._last_log_time > 0.5:
                    motor_cx_oe._last_log_time = now
                    # 필터 전후 차이 계산
                    diff_cx = abs(box_cx - filtered_motor_cx)
                    diff_cy = abs(box_cy - filtered_motor_cy)
                    # 속도 계산
                    speed_cx = abs(motor_cx_oe.dx_prev) if hasattr(motor_cx_oe, 'dx_prev') else 0
                    speed_cy = abs(motor_cy_oe.dx_prev) if hasattr(motor_cy_oe, 'dx_prev') else 0
                    print(f"\n{'='*70}")
                    print(f"🔍 필터 진단 (모터)")
                    print(f"{'='*70}")
                    print(f"📍 원본 좌표: ({box_cx}, {box_cy})")
                    print(f"📍 필터 후:   ({filtered_motor_cx}, {filtered_motor_cy})")
                    print(f"📏 필터 차이: X={diff_cx}px, Y={diff_cy}px")
                    print(f"🏃 현재 속도: X={speed_cx:.1f}px/s, Y={speed_cy:.1f}px/s")
                    print(f"⚙️  필터 설정: min_cutoff={motor_cx_oe.min_cutoff}, beta={motor_cx_oe.beta}")
                    print(f"{'='*70}\n")

                # compute_motor_angles_safe가 이미 Soft deadzone + Adaptive EMA로 부드럽게 처리
